 ...
 for my $cv(@cvs){
 	...
 	my $ccle_file=glob "$cv/CCLE_expression_copy_train.txt";
 	my $cor_file=glob "$cv/pearson_matrix_train.txt";
 	
 	open ACH,$ach_file or die $!; # Achilles
 	open CCLE,$ccle_file or die $!; # Expression and Copy number
 	...
 	my @ach=();  #gene names in ach 
 	my @ccle=();  # gene names in exp and cn 
 	my @exp = ();
	...
	while(<CCLE>){
 	...
 	while(<COR>){
 		...
 		my @scores = split /,/,$_;
 		my $size_of_exp = @exp;
 		@scores = @scores[$size_of_exp..$#scores];
 		print("$#scores\nand $#ccle\n$size_of_exp\n");
